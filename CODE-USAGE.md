...
> 仓库地址：https://github.com/1589572071-bot/video
...
更完整的架构说明见 [Project-Documentation.md](docs/Project-Documentation.md)，产品需求见 [MetaCut-PRD-Refined.md](docs/MetaCut-PRD-Refined.md)。
...
```bash
git clone https://github.com/1589572071-bot/video.git
cd video
npm install
```
...
本机浏览器打开 http://localhost:3000；同一局域网设备打开
`http://<运行机器的局域网 IP>:3000`。默认监听 `0.0.0.0:3000`。
...
1. 上传样例爆款视频（≤ 50MB，**建议时长约 30 秒**，约 2 个 15s 渲染区块）
2. 可选填写基础信息
...
7 个 Prompt 位点位于 `docs/prompts/01`–`07`，详见 [docs/prompts/README.md](docs/prompts/README.md)。
...
为兼顾解析与渲染稳定性，**建议控制在 30 秒左右**（约 2 个渲染区块）。系统理论上支持更长视频，并会按 **15s/块** 继续拆分，但时长增加后，豆包 VLM 解析、万相异步生视频、FFmpeg 拼接等环节更容易超时或失败，演示环境尤甚。这属于已知运行边界，而非功能缺陷。
...
- [Project-Documentation.md](docs/Project-Documentation.md) — 架构与自主设计说明
- [MetaCut-PRD-Refined.md](docs/MetaCut-PRD-Refined.md) — 产品需求
- [docs/prompts/README.md](docs/prompts/README.md) — Prompt 资产管理
