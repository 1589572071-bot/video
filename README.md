...
本机打开 http://localhost:3000；同一局域网设备打开
`http://<运行机器的局域网 IP>:3000`。开发和生产脚本默认监听
`0.0.0.0:3000`。

不填写模型密钥也能启动并浏览工作台 UI；要跑通“视频解析 → 剧本生成”，
至少需要在 `.env.local` 配置 `DOUBAO_APIKEY` 和 `DOUBAO_EP`。真实视频渲染
还需要百炼配置；未配置百炼时渲染自动进入 Mock 模式。完整说明见
[代码使用说明](CODE-USAGE.md)。
