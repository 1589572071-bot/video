...
| 项 | 值 |
|----|----|
| 环境变量 | `DOUBAO_APIKEY`、`DOUBAO_EP`（与 Step1 共用） |
| 关联 Schema | `product/v1` |
| 对应 Pipeline | Step 2 · 商品特征提取 |
| 输入模态 | 文本 + 图片 + 视频（至少一项） |
...
- 当前前端已支持文本（一句话）+ 多图 + 视频三种输入，符合"图像 > 视频 > 文本"的内容权重优先级。
- 点击"开始多模态拆解"时，前端会先校验至少输入一项。
- 后端调用豆包多模态接口（`image_url` / `video_url` + 文本），将用户输入拼装后送入此 Prompt。
- 新 Prompt 强调原子级提取、差异化描述与动态推理，严禁模板化输出。
